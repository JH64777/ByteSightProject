...
def BoardPage(request, boardCategory, pageNumber): # 게시판
    ...
    variables = {'loggedin' : False, 'posts' : data, 'page' : pageNumber, 'boardtitle' : boardTitle, 'category' : boardCategory}
...
def SubmitPost(request, boardCategory): # 게시글 DB에 반영
    ...
    author = Accounts.objects.get(nickname="root") # ****나중에 로그인 기능 구현하면 여기다가 게시자 id 넣을 것****
    
    postid = Hashing("root" + str(timezone.now())) # 게시글 식별 코드 만드는 부분
